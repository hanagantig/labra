The PartFile contains the following files:
...
For more details about the LoincPartLink files visit our knowledge base  - https://loinc.org/kb/enriched-linkages-between-loinc-terms-and-loinc-parts/

Note: Because of the very large number of rows, the LoincPartLink_Supplemenatry.csv exceeds the maximum row limit of many common spreadsheet applications.
...
Overview:
This file contains every LOINC term included in the current LOINC release and their Primary, Radiology, and Document Ontology part link types. Link types are defined as follows:  
1. "Primary" - the key parts that make up the five (or optionally six) main attributes of the LOINC Fully Specified Name;  
2. "DocumentOntology" - the parts linked to a LOINC term that define that term within the Document Ontology; 
3. "Radiology" - the parts linked to a LOINC term that define that term within the LOINC/RSNA Radiology Playbook unified model;

Note that Document Ontology and Radiology terms have "Primary" part links (Component, Property, etc.) as well as a set of "DocumentOntology" or "Radiology" part links, respectively.
...
Columns:
 ...
 - LinkTypeName - the type of link between the LOINC term and the Part in a given row. Values include "Primary", "DocumentOntology", and "Radiology" as described above
 - Property - the attribute that the Part represents

Sort order:
This file is sorted by alphabetically by the LoincNumber; then by the LinkTypeName in the following order (note that not all terms will have all of these links): "Primary", "DocumentOntology", and "Radiology"; and finally by the PartNumber.

------------
LoincPartLink_Supplementary.csv

Overview:
This file contains every LOINC term included in the current LOINC release and their part link types other than Primary, Radiology, and Document Ontology. Link types in this file include:  
1. "Search" - parts that are only linked to a given term in order to facilitate efficient searching and location of that term (note that parts that have a "Search" type link to one LOINC term may have a "Primary" link to another LOINC term); 
2. "DetailedModel" - the parts that comprise all of the more detailed compositional subparts that create a fully defined term according to the LOINC semantic model;
3. "SyntaxEnhancement" - the parts linked to a LOINC term that represent useful pieces of the LOINC term name that are not officially part of the semantic model;
4. "SemanticEnhancement" - the parts linked to a LOINC term that are part of the LOINC term name but not officially part of the semantic model and are used to link terms to external sources or coding systems; and
5. "Metadata" - parts that define accessory term attributes that are coded but not part of the LOINC term name (e.g. CLASS).

Note that Document Ontology and Radiology terms will not have "DetailedModel" links because the general detailed model is superseded by the Document Ontology and Radiology models.
...
Columns:
 ...
 - PartCodeSystem - the coding system to which the Part belongs
 - PartTypeName - the type of Part. Examples: Component, Method
 - LinkTypeName - the type of link between the LOINC term and the Part in a given row. Values include "Search", "DetailedModel", "DetailedModel", "SemanticEnhancement", and "Metadata" as described above
 - Property - the attribute that the Part represents

Sort order:
This file is sorted by alphabetically by the LoincNumber; then by the LinkTypeName in the following order (note that not all terms will have all of these links): "DetailedModel", "SyntaxEnhancement", "SemanticEnhancement", "Search", and "Metadata"; and finally by the PartNumber.
...
Columns:
 ...
 - Equivalence - an indicator of the specificity of mapping between the LOINC part and the external concept, from the point of view of the external ("target") concept. Equivalent matches are represented as "equivalent"; maps in which the target concept is more general than the LOINC concept it is mapped to are represented as "wider"; and maps in which the target concept is more granular than the LOINC concept it is mapped to are represented as "narrower". These values are based on the HL7 FHIR ConceptMap model.
